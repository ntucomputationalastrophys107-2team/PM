...
// KERNEL PoissonEquationInK_PerBC_GPU: Calculate phi(k) in K-space for periodic BC by GPU
__global__
void PoissonEquationInK_PerBC_GPU( const cufftDoubleComplex *g_rho_K, cufftDoubleComplex *g_phi_K ){

    const uint  i    = blockDim.x*blockIdx.x + threadIdx.x;
    if( i==0 ){
        g_phi_K[i].x = 0.0;       // set DC to 0
        g_phi_K[i].y = 0.0;       // set DC to 0
    }
    else if( i<N*N*(N/2+1) ){
        int nx = (i/(N/2+1))/N - (((i/(N/2+1))/N)/(N/2))*N;
        int ny = (i/(N/2+1))%N - (((i/(N/2+1))%N)/(N/2))*N;
        int nz = i%(N/2+1);
        double KK = (nx*2*M_PI/L)*(nx*2*M_PI/L) + (ny*2*M_PI/L)*(ny*2*M_PI/L) + (nz*2*M_PI/L)*(nz*2*M_PI/L); // K*K = Kx*Kx + Ky*Ky + Kz*Kz
        g_phi_K[i].x = g_rho_K[i].x/KK*4.0*M_PI*G*(-1.0/(N*N*N)); // real part phi(k) = -4*Pi*G*rho(k)/k^2 and normalize by 1/N^3
        g_phi_K[i].y = g_rho_K[i].y/KK*4.0*M_PI*G*(-1.0/(N*N*N)); // imag part phi(k) = -4*Pi*G*rho(k)/k^2 and normalize by 1/N^3
    }
}// KERNEL PoissonEquationInK_PerBC_GPU


// KERNEL PoissonEquationInK_IsoBC_GPU: Calculate phi(k) in K-space for isolated BC by GPU
__global__
void PoissonEquationInK_IsoBC_GPU( const cufftDoubleComplex *g_mas_0pad_K, const cufftDoubleComplex *g_greensfn_K, cufftDoubleComplex *g_phi_0pad_K ){

    const uint  i    = blockDim.x*blockIdx.x + threadIdx.x;
    if( i<(2*N)*(2*N)*(N+1) ){
        g_phi_0pad_K[i].x = G*( g_mas_0pad_K[i].x*g_greensfn_K[i].x-g_mas_0pad_K[i].y*g_greensfn_K[i].y)*(1.0/(2*N*2*N*2*N));// real part phi(k) = G*mas(k)*gre(k) and normalize by 1/(2N)^3
        g_phi_0pad_K[i].y = G*( g_mas_0pad_K[i].x*g_greensfn_K[i].y+g_mas_0pad_K[i].y*g_greensfn_K[i].x)*(1.0/(2*N*2*N*2*N));// imag part phi(k) = G*mas(k)*gre(k) and normalize by 1/(2N)^3
    }
}// KERNEL PoissonEquationInK_IsoBC_GPU
...
#       ifdef GPU
        ...
        cufftDoubleReal *d_rho;       // (device) density
        cudaMalloc( &d_rho, sizeof(cufftDoubleReal)*N*N*N );
        cufftDoubleReal *d_phi;       // (device) potential
        cudaMalloc( &d_phi, sizeof(cufftDoubleReal)*N*N*N );
        cufftDoubleComplex *d_rho_K;  // (device) density   in K space
        cudaMalloc( &d_rho_K, sizeof(cufftDoubleComplex)*N*N*(N/2+1) );
        cufftDoubleComplex *d_phi_K;  // (device) potential in K space
        cudaMalloc( &d_phi_K, sizeof(cufftDoubleComplex)*N*N*(N/2+1) );

        cufftHandle rhoXtorhoK, phiKtophiX;
        cufftPlan3d( &rhoXtorhoK, N, N, N, CUFFT_D2Z ); // Fourier Transform from rho(x) to rho(k)
        cufftPlan3d( &phiKtophiX, N, N, N, CUFFT_Z2D ); // Inverse Fourier Transform from phi(k) to phi(x)

        cudaMemcpy( d_rho, rho, N*N*N*sizeof(double), cudaMemcpyHostToDevice );

        cufftExecD2Z( rhoXtorhoK, d_rho, d_rho_K ); // Fourier Transform from rho(x) to rho(k)
        PoissonEquationInK_PerBC_GPU <<< GRID_SIZE, BLOCK_SIZE  >>>( d_rho_K, d_phi_K );
        cufftExecZ2D( phiKtophiX, d_phi_K, d_phi ); // Inverse Fourier Transform from phi(k) to phi(x)
        cudaMemcpy( phi, d_phi, N*N*N*sizeof(double), cudaMemcpyDeviceToHost );

        cufftDestroy( rhoXtorhoK );
        cufftDestroy( phiKtophiX );
...
#       ifdef GPU
        ...
        double *h_mas_0pad = new double[2*N*2*N*2*N]; // (host) zero padding on the mass array
        double *h_greensfn = new double[2*N*2*N*2*N]; // (host) symmetric discrete Green's function -1/r
        double *h_phi_0pad = new double[2*N*2*N*2*N]; // (host) zero padding ont eh potential array
#       pragma omp parallel for collapse( 3 ) num_threads(NThread)
        for(int i=0;i<2*N;i++){
        for(int j=0;j<2*N;j++){
        for(int k=0;k<2*N;k++){
            int nx,ny,nz;              // number of distance interval
            if( i<N && j<N && k<N ) h_mas_0pad[k+(2*N)*(j+(2*N)*i)] = rho[Index( i, j, k )]*dx*dx*dx;  // mass of cell = density * cell volume
            else h_mas_0pad[k+(2*N)*(j+(2*N)*i)] = 0.0;                                       // zero padding

            if( i>=N ) nx=2*N-i; else nx=i;  // symmetrization
            if( j>=N ) ny=2*N-j; else ny=j;  // symmetrization
            if( k>=N ) nz=2*N-k; else nz=k;  // symmetrization

            if( i==0 && j==0 && k==0 ) h_greensfn[0] = 0.0;  // ignore self force
            else h_greensfn[k+(2*N)*(j+(2*N)*i)] = -1.0/(dx*sqrt( nx*nx + ny*ny + nz*nz )); // -1/r
        }}}
        cufftDoubleReal *d_mas_0pad;          // (device) zero padding on the mass array
        cudaMalloc( &d_mas_0pad, sizeof(cufftDoubleReal)*(2*N)*(2*N)*(2*N) );
        cufftDoubleReal *d_greensfn;          // (device) symmetric discrete Green's function -1/r
        cudaMalloc( &d_greensfn, sizeof(cufftDoubleReal)*(2*N)*(2*N)*(2*N) );
        cufftDoubleReal *d_phi_0pad;          // (device) zero padding on the potential array
        cudaMalloc( &d_phi_0pad, sizeof(cufftDoubleReal)*(2*N)*(2*N)*(2*N) );
        cufftDoubleComplex *d_mas_0pad_K;     // (device) mass in K space
        cudaMalloc( &d_mas_0pad_K, sizeof(cufftDoubleComplex)*(2*N)*(2*N)*(N+1) );
        cufftDoubleComplex *d_greensfn_K;     // (device) Green's function in K space
        cudaMalloc( &d_greensfn_K, sizeof(cufftDoubleComplex)*(2*N)*(2*N)*(N+1) );
        cufftDoubleComplex *d_phi_0pad_K;     // (device) potential in K space
        cudaMalloc( &d_phi_0pad_K, sizeof(cufftDoubleComplex)*(2*N)*(2*N)*(N+1) );

        cufftHandle masXtomasK, greXtogreK, phiKtophiX;
        cufftPlan3d( &masXtomasK, 2*N, 2*N, 2*N, CUFFT_D2Z ); // Fourier Transform from mas(x) to mas(k)
        cufftPlan3d( &greXtogreK, 2*N, 2*N, 2*N, CUFFT_D2Z ); // Fourier Transform from gre(x) to gre(k)
        cufftPlan3d( &phiKtophiX, 2*N, 2*N, 2*N, CUFFT_Z2D ); // Inverse Fourier Transform from phi(k) to phi(x)

        cudaMemcpy( d_mas_0pad, h_mas_0pad, 2*N*2*N*2*N*sizeof(double), cudaMemcpyHostToDevice );
        cudaMemcpy( d_greensfn, h_greensfn, 2*N*2*N*2*N*sizeof(double), cudaMemcpyHostToDevice );

        cufftExecD2Z( masXtomasK, d_mas_0pad, d_mas_0pad_K ); // Fourier Transform from mas(x) to mas(k)
        cufftExecD2Z( greXtogreK, d_greensfn, d_greensfn_K ); // Fourier Transform from gre(x) to gre(k)
        PoissonEquationInK_IsoBC_GPU <<< GRID_SIZE, BLOCK_SIZE >>> ( d_mas_0pad_K, d_greensfn_K, d_phi_0pad_K );
        cufftExecZ2D( phiKtophiX, d_phi_0pad_K, d_phi_0pad ); // Inverse Fourier Transform from phi(k) to phi(x)

        cudaMemcpy( h_phi_0pad, d_phi_0pad, 2*N*2*N*2*N*sizeof(double), cudaMemcpyDeviceToHost );
#       pragma omp parallel for collapse( 3 ) num_threads(NThread)
        for(int i=0;i<N;i++)
        for(int j=0;j<N;j++)
        for(int k=0;k<N;k++)
            phi[Index( i, j, k )] = h_phi_0pad[k+(2*N)*(j+(2*N)*i)]; // remove the padding

        cufftDestroy( masXtomasK );
        cufftDestroy( greXtogreK );
        cufftDestroy( phiKtophiX );

        cudaFree( d_mas_0pad_K );
        cudaFree( d_greensfn_K );
        cudaFree( d_phi_0pad_K );
        cudaFree( d_mas_0pad );
        cudaFree( d_greensfn );
        cudaFree( d_phi_0pad );
...
// KERNEL Acceleration_DN_GPU: Calculate the acceleration for Direct N-Body by GPU
__global__
void Acceleration_DN_GPU( const double *g_m, const double *g_x, double *g_a )
{

    const uint  i    = blockDim.x*blockIdx.x + threadIdx.x;
    if( i<ParN ){
//  load data from global memory to the **per-thread** registers (much faster)
    const double I_x = g_x[i*3+0];
    const double I_y = g_x[i*3+1];
    const double I_z = g_x[i*3+2];

//  declared shared-memory arrays
    __shared__ double sJ_x[BLOCK_SIZE];
    __shared__ double sJ_y[BLOCK_SIZE];
    __shared__ double sJ_z[BLOCK_SIZE];
    __shared__ double sJ_m[BLOCK_SIZE];

    double I_a[3] = { 0.0, 0.0, 0.0 };
    double dx, dy, dz, r, r3;
    uint  j;

//  calculate BLOCK_SIZE j particles at a time (because of the limited shared memory)
    for(int J_Base=0;J_Base<ParN;J_Base+=BLOCK_SIZE){

        j = J_Base + threadIdx.x;
        sJ_x[threadIdx.x] = g_x[j*3+0];
        sJ_y[threadIdx.x] = g_x[j*3+1];
        sJ_z[threadIdx.x] = g_x[j*3+2];
        sJ_m[threadIdx.x] = g_m[j];
...
//          store temporary data in the per-thread variable "Acc" instead of g_Acc (much faster)
            I_a[0] += G*sJ_m[k]*dx/r3;
            I_a[1] += G*sJ_m[k]*dy/r3;
            I_a[2] += G*sJ_m[k]*dz/r3;
...
//  allocate device memory
    double *d_x, *d_a, *d_m;
    cudaMalloc( &d_x, ParN*3*sizeof(double) );
    cudaMalloc( &d_a, ParN*3*sizeof(double) );
    cudaMalloc( &d_m, ParN*sizeof(double) );

    cudaMemcpy( d_x, x, ParN*3*sizeof(double), cudaMemcpyHostToDevice );
    cudaMemcpy( d_m, ParM, ParN*sizeof(double), cudaMemcpyHostToDevice );

    Acceleration_DN_GPU <<< GRID_SIZE, BLOCK_SIZE >>> ( d_m, d_x, d_a );

    cudaMemcpy( a, d_a, ParN*3*sizeof(double), cudaMemcpyDeviceToHost );
